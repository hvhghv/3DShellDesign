...
- 视口为选中的面板或接口提供安装面内的移动和缩放手柄，拖动结束后同步参数检查与浏览器项目缓存。
...
- 包含全部面板实例的多零件 3MF、逐面板 SVG/DXF、BOM CSV 和制造清单 JSON 导出。
...
Three.js 场景仍用于实时预览，STL 和 3MF 则由 Manifold WASM 根据参数重新生成封闭实体。浏览器回归会检查 WebGL 像素、视口移动/缩放手柄、面板和接口多实例缓存、三类 STL 尺寸、3MF 包结构、面板 DXF、BOM、KiCad、Gerber/Excellon、STEP Worker/WASM，以及蜂窝、天线、翻盖、滑盖、热熔螺母、圆形接口和壁挂安装耳等非默认实体路径。
