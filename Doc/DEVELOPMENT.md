...
- 螺丝、磁吸、卡扣、滑盖和转轴翻盖五种固定方式的可交互预览与制造实体；磁铁承托支持双壁角托、单壁耳台、四周内翻边和底板连续立柱，磁铁槽保留连续槽底。
...
- 视口为选中的面板和接口提供安装面内的移动/缩放手柄，为天线提供移动手柄；拖动结束后同步参数检查与浏览器项目缓存。
- 面板、接口和天线采用互不混杂的上下文检查器；面内拖动、数值输入和旧缓存恢复统一约束到安装面安全区域，对象树右键菜单可删除对应实例。
- 独立亚克力、PC、ABS 和金属面板，以及螺丝、磁吸、滑轨面板固定；面板换面时自动适配目标表面的常用尺寸。
- USB、电源、网络、FPC、四种间距乘三种位数的端子和四类天线均采用数据驱动制造定义；对象树加号先打开可搜索的分类选择器，选定型号后才创建实例，并检查面板边界、同类重叠及天线与接口冲突。
