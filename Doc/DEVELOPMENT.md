...
- 接线端子、排针、FPC、薄膜按键、金属按钮、拨动开关、带线 LED 指示灯、OLED/LCD 显示屏和显示窗口。
- 风扇、散热孔、线缆孔和护线圈。
...
- 首版以桌面 Chromium/Edge/Firefox 为主要目标；窄屏采用对象树抽屉、完整工具菜单和上下分区检查器，支持选择、添加、删除及参数编辑，复杂 3D 手柄操作仍建议使用桌面指针设备。
...
- 螺丝、磁吸、隐藏卡扣、按压快拆扣、滑盖、转轴翻盖和双侧快拆销七种顶盖固定方式的可交互预览与制造实体；快拆扣包含侧壁释放窗口，快拆销包含两侧对合销轴套；磁铁槽保留连续槽底。
- 可更换面板采用稳定 ID 的多实例数组；左侧对象树提供直接添加入口，每个实例可独立选择六个安装面、尺寸、偏移、材料和固定方式，旧版单面板参数在载入时自动迁移。
- 面板增加受壳体表面厚度约束的嵌入深度、圆角、嵌入边框宽度及横纵螺丝边距；预览和 Manifold 制造实体均生成全尺寸浅槽、较小通孔与至少 0.4 mm 承托层。
- 面板支持螺丝、磁吸、弹性柱卡扣和滑轨；螺丝使用与安装面同厚、连接保留边框且带导向孔的一体平面舌片，不生成向壳内突出的面板螺柱；磁吸在面板背面与壳体生成对应盲孔，卡扣在面板背面生成一体卡柱并在壳体生成承接孔，位置同步到预览、SVG、DXF 和 STL。
- 顶盖螺丝固定支持独立的螺丝头沉孔开关与深度；预览将低头螺丝顶面放置为与外表面齐平，Manifold 顶盖和面板实体实际减去沉孔，深度约束会保留至少 0.4 mm 承托层，SVG/DXF 使用独立 `POCKET` 标记输出面板沉孔。
- 接口采用稳定 ID 的多实例数组；左侧对象树提供直接添加入口，每个实例可独立选择器件、六个壳体面或指定面板实例、面内偏移、开孔尺寸和 0/90/180/270 度方向；旧版单前壁接口参数在载入时自动迁移。
- 天线采用稳定 ID 的多实例数组；每个实例可独立选择类型、六个壳体面或指定面板、面内偏移、旋转和穿板孔径，旧版单后壁天线参数在载入时自动迁移。
- PCB 参考采用稳定 ID 的多实例数组；KiCad、Gerber/Excellon 和 STEP 导入均追加实例，每块板拥有独立 X/Z 偏移、抬高和 0/90/180/270 度旋转，并共同驱动预览螺柱、制造螺柱和内腔边界检查。
- 自定义组件支持参数化长方体、圆柱体和 STEP/STL/OBJ 网格包络；实例可编辑尺寸、三轴位置、三轴旋转和颜色，导入网格保存在 IndexedDB，项目 JSON 保留来源与包络参数。
- 电池仓支持 AAA、AA、18650、软包锂电和自定义规格；实例可编辑槽数、仓体尺寸、挡边高度、壁厚、间隙、面内偏移和旋转，仓壁与多槽隔板并入下壳的单连通 Manifold 制造实体并写入 BOM。
- 视口为选中的面板和接口提供安装面内的移动/缩放手柄，为 PCB、天线和电池仓提供移动手柄；拖动过程中持续渲染，释放手柄时立即同步参数检查与浏览器项目缓存。
- 左侧顶盖节点提供半透明检查模式，并可分别隐藏顶盖、底板、前后壁和左右壁；六面显隐只裁剪 Three.js 检查视图，不进入项目制造参数或 STL/3MF 导出。透明材料关闭深度写入，非嵌入透明面板与安装面保留微小显示间隙，工作平面切换使用极角滞回以减少底视图深度闪烁。
- 面板、接口和天线采用互不混杂的上下文检查器；面内拖动、数值输入和旧缓存恢复统一约束到安装面安全区域，对象树右键菜单可删除对应实例。
- 对象树支持按对象名称、类型和安装面即时筛选；六类实例可从右键菜单或 `Ctrl+D` 完整复制，复制接口会按开孔宽度自动错开。编辑历史保留 60 个步骤，连续拖动或连续数字输入合并为一步，工具栏及 `Ctrl+Z` / `Ctrl+Y` 提供撤销和重做。
- 六类实例支持独立的检查视图显隐和编辑锁定；隐藏不会进入项目制造参数或改变 STL/3MF，锁定会同时阻止 3D 变换、检查器参数更新与删除。对象树显示状态图标，检查器状态条可直接显示或解锁，一键显示全部对象用于恢复复杂检查场景。
- 数值输入保留编辑中的临时文本，合法数字即时驱动 3D，失焦或回车后按上下限提交；切换对象或检查器标签时滚动位置自动回到顶部。
- 低于 850 px 时对象树以遮罩抽屉呈现，低于 640 px 时项目/导入/视图命令汇总到完整工具菜单；接口和天线选择器使用居中模态层并支持空白处或 Escape 关闭。
- 面板可使用亚克力、PC、ABS、金属板材或 PETG/ABS/PA 等打印材料，并支持螺丝、双侧盲孔磁吸、一体弹性柱卡扣和滑轨固定；面板换面时自动适配目标表面的常用尺寸。
...
Three.js 场景仍用于实时预览，STL 和 3MF 则由 Manifold WASM 根据参数重新生成封闭实体。浏览器回归会检查 WebGL 像素、视口移动/缩放手柄、面板、接口和天线多实例缓存、三类 STL 尺寸、3MF 包结构、带天线开孔的面板 DXF、BOM、KiCad、Gerber/Excellon、STEP Worker/WASM，以及蜂窝、翻盖、滑盖、热熔螺母、圆形接口和壁挂安装耳等非默认实体路径。

当前明确限制：STEP/STL/OBJ 网格会保存在当前浏览器的 IndexedDB 中，但不写入项目 JSON；复杂导入模型只做可定位、可缩放的包络显示参考，不自动推断开孔或并入外壳布尔实体；连接器和天线库使用通用机械包络，生产前仍需按具体器件图纸及射频测试复核；制造规则是建议和告警，不构成材料强度、防水、射频或安全认证。
