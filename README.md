...
- 将可更换面板放置到顶盖、底板或任一侧壁，并设置面内偏移。
- 添加多个 USB、Type-C、RJ45、电源、端子或 FPC 接口；每个接口可独立选择六个壳体面或可更换面板、位置与旋转方向。
- 生成圆孔、长槽或蜂窝底部镂空阵列并检查边界与剩余筋宽。
- 创建独立亚克力、PC、ABS 或金属面板，支持螺丝、磁吸和滑轨固定。
- 从数据驱动库选择 USB-A、Micro USB、Type-C、DC、RJ45、接线端子和 FPC。
- 配置 SMA、RP-SMA、内贴 FPC 或 PCB 板边天线，并生成后壁开孔、射频禁入区和 BOM。
...
- 导出独立二进制 STL、多零件 3MF、面板 SVG/DXF、BOM CSV 和制造清单 JSON。
