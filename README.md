...
- 点击接口加号后从可搜索、分类的器件库选择型号，再添加多个 USB、Type-C、RJ45、电源、端子或 FPC 接口；FPC 默认用间距和针数两项紧凑选择，也可搜索具体型号；每个接口可独立选择六个壳体面或可更换面板、位置与旋转方向。
...
- 从数据驱动库选择 USB-A、Micro USB、Type-C、DC、RJ45、`0.5/1.0 mm` 间距且覆盖 `5P–40P` 的 FPC，以及 1.0/1.25/2.54/5.08 mm 间距的 2P/4P/5P 端子。
